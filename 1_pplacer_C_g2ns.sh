...
BASE_DIR="data/genes/${DATASET}/${GENE_DIR}"
SUBJECT_FILE="G2_NS_station_6tr_${subject}.extendedFrags.6tr.orfs40.fasta.gz"

echo "searching profile against sequence database for each subject file"
echo "cut-off: $CUTOFF"
module purge; ml hmmer
hmmsearch -A "${BASE_DIR}/${GENE}.${subject}.query.sto" -T ${CUTOFF} --incT ${CUTOFF} --cpu ${NCORES} --tblout "${BASE_DIR}/${GENE}.${subject}.hmm_out.tab" "${BASE_DIR}/${GENE}.hmm" "/blue/b.durham/rebeccakey/2_metaTJiwoon/${SUBJECT_DIR}/${SUBJECT_FILE}"
...
echo "Converting alignment file format from Stockholm back Fasta using seqmagick"
...
pplacer -c "${BASE_DIR}/${GENE}.refpkg" --keep-at-most 1 --max-pend $MAXPEND "${BASE_DIR}/${GENE}.${subject}.aln.fasta" -o "results/${DATASET}/${GENE_DIR}/${GENE_DIR}.${subject}.aln.jplace"
